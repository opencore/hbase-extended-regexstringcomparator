 ...
 *     new ExtendedRegexStringComparator(
 *       // v4 IP address
 ...
 * </pre>
 * <p>
 * Supports {@link Pattern} flags as well:
 * <p>
 * <pre>
 * ValueFilter vf = new ValueFilter(CompareOp.EQUAL,
...
public class ExtendedRegexStringComparator extends ByteArrayComparable {

  private static final Log LOG = LogFactory.getLog(ExtendedRegexStringComparator.class);
  ...
  public ExtendedRegexStringComparator(String expr, int flags, EngineType engine) {
    ...
    switch (engine) {
      ...
      case JONI:
        ...
        break;
      case RE2J:
        this.engine = new Re2JRegexEngine(expr, flags);
        break;
      case BRICS:
        this.engine = new BricsRegexEngine(expr, flags);
        break;
      case FAST_BRICS:
        this.engine = new FastBricsRegexEngine(expr, flags, false);
        break;
      case FASTEST_BRICS:
        this.engine = new FastBricsRegexEngine(expr, flags, true);
        break;
    }
  }
  ...
  public static ExtendedRegexStringComparator parseFrom(final byte[] pbBytes)
    ...
    ExtendedRegexStringComparator comparator;
    if (proto.hasEngine()) {
      EngineType engine = EngineType.valueOf(proto.getEngine());
      comparator = new ExtendedRegexStringComparator(proto.getPattern(), proto.getPatternFlags(),
        engine);
    } else {
      comparator = new ExtendedRegexStringComparator(proto.getPattern(), proto.getPatternFlags());
    }
  ...
  private abstract static class BaseRegexEngine implements Engine {
    ...
    BaseRegexEngine(String regex, int flags, EngineType engineType) {
      this.regex = regex;
      this.flags = flags;
    ...
    public byte[] toByteArray() {
      ...
      builder.setEngine(engineType.name());
    ...
    public int compareTo(byte[] value, int offset, int length) {
      ...
      return doMatch(tmp) ? 0 : 1;
    }

    protected abstract boolean doMatch(String matchString);
  }
  ...
  static class JavaRegexEngine extends BaseRegexEngine {
    ...
    private JavaRegexEngine(String regex, int flags) {
      super(regex, flags, EngineType.JAVA);
    ...
    protected boolean doMatch(String matchString) {
      ...
      return pattern.matcher(matchString).find();
    }
  ...
  static class JoniRegexEngine implements Engine {
    ...
    private JoniRegexEngine(String regex, int flags) {
      this.regex = regex;
      byte[] b = Bytes.toBytes(regex);
      pattern = new Regex(b, 0, b.length, patternToJoniFlags(flags), encoding, Syntax.Java);
    }
    ...
    private static int joniToPatternFlags(int flags) {
      int newFlags = 0;
  ...
  static class Re2JRegexEngine extends BaseRegexEngine {
    ...
    private Re2JRegexEngine(String regex, int flags) {
      super(regex, flags, EngineType.RE2J);
      pattern = com.google.re2j.Pattern.compile(regex, flags);
    }
  ...
  static class BricsRegexEngine extends BaseRegexEngine {
    ...
    private BricsRegexEngine(String expr, int flags) {
      super(expr, flags, EngineType.BRICS);
      RegExp regExp = new RegExp(expr, flags);
  ...
  static class FastBricsRegexEngine extends BaseRegexEngine {
    ...
    private FastBricsRegexEngine(String expr, boolean tableize) {
      this(expr, RegExp.ALL, tableize);
    }

    private FastBricsRegexEngine(String expr, int flags, boolean tableize) {
      super(expr, flags, EngineType.FAST_BRICS);
      RegExp regExp = new RegExp(expr, flags);
      automaton = new RunAutomaton(regExp.toAutomaton(), tableize);
    }
